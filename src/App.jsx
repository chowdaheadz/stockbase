...
// STATUS_STYLES and PO_STATUS are now defined inside App() so they can react to the theme.
...
const saveInv    = async (d) => { try { await dbSet("inventory", d); } catch {} };
  ...
  // ── STYLES ────────────────────────────────────────────────────────────────
  const C = theme === 'dark' ? {
    bg: '#0a0d14',       surface: '#0f1420',     border: '#1e2d45',
    muted: '#475569',    dim: '#64748b',          text: '#e2e8f0',
    amber: '#f59e0b',    blue: '#60a5fa',         green: '#4ade80',
    orange: '#f97316',   red: '#ef4444',          purple: '#a78bfa',
    surfaceDeep: '#060910', tdBorder: '#0f1e30',  btnText: '#94a3b8',
    navActive: '#f59e0b18', dangerBg: '#450a0a',  dangerBorder: '#7f1d1d',
    blueBg: '#1e3a5f',   successBg: '#14532d',   errorRowBg: '#1a0505',
    dangerBgSubtle: '#450a0a40', warnBgSubtle: '#451a0340', dangerBorderSubtle: '#7f1d1d40',
  } : {
    bg: '#f8fafc',       surface: '#ffffff',      border: '#e2e8f0',
    muted: '#94a3b8',    dim: '#64748b',          text: '#0f172a',
    amber: '#d97706',    blue: '#2563eb',         green: '#16a34a',
    orange: '#ea580c',   red: '#dc2626',          purple: '#7c3aed',
    surfaceDeep: '#f1f5f9', tdBorder: '#e8edf2',  btnText: '#64748b',
    navActive: '#d9770618', dangerBg: '#fef2f2',  dangerBorder: '#fca5a5',
    blueBg: '#eff6ff',   successBg: '#dcfce7',   errorRowBg: '#fef2f2',
    dangerBgSubtle: '#fef2f2', warnBgSubtle: '#fff7ed', dangerBorderSubtle: '#fca5a5',
  };
  const STATUS_STYLES = theme === 'dark' ? {
    out: { label: 'OUT OF STOCK', bg: '#450a0a', color: '#f87171', dot: '#ef4444' },
    low: { label: 'LOW STOCK',    bg: '#451a03', color: '#fb923c', dot: '#f97316' },
    ok:  { label: 'OK',           bg: '#052e16', color: '#4ade80', dot: '#22c55e' },
  } : {
    out: { label: 'OUT OF STOCK', bg: '#fef2f2', color: '#dc2626', dot: '#dc2626' },
    low: { label: 'LOW STOCK',    bg: '#fff7ed', color: '#ea580c', dot: '#ea580c' },
    ok:  { label: 'OK',           bg: '#f0fdf4', color: '#16a34a', dot: '#16a34a' },
  };
  const PO_STATUS = theme === 'dark' ? {
    ...
    sent:     { label: 'SENT TO SUPPLIER', bg: '#eff6ff', color: '#2563eb' },
  ...
  const s = {
    ...
    navBtn: (a) => ({ padding: "6px 14px", border: "none", background: a ? C.navActive : "transparent", color: a ? C.amber : C.btnText, borderRadius: 6, cursor: "pointer", fontFamily: "inherit", fontSize: 13, fontWeight: 600, borderBottom: a ? `2px solid ${C.amber}` : "2px solid transparent", transition: "all 0.15s" }),
    ...
    td: { padding: "11px 12px", fontSize: 13, borderBottom: `1px solid ${C.tdBorder}`, verticalAlign: "middle" },
    ...
    btn: (v) => ({ padding: v==="primary"?"10px 18px":"6px 13px", background: v==="primary"?C.amber:v==="danger"?C.dangerBg:v==="blue"?C.blueBg:C.border, color: v==="primary"?C.bg:v==="danger"?C.red:v==="blue"?C.blue:C.btnText, border:"none", borderRadius:6, cursor:"pointer", fontSize:12, fontWeight:700, fontFamily:"inherit", letterSpacing:0.5, transition:"all 0.15s", whiteSpace:"nowrap" }),
    ...
    poBadge: (st) => ({ display:"inline-flex", alignItems:"center", gap:5, padding:"3px 10px", borderRadius:4, fontSize:10, fontWeight:700, letterSpacing:1, background:PO_STATUS[st]?.bg||C.border, color:PO_STATUS[st]?.color||C.btnText }),
...
// Password screen
  if (!authed) return (
    <div style={{background:C.bg,minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"'DM Sans','Segoe UI',sans-serif"}}>
      <div style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:16,padding:"48px 40px",width:360,textAlign:"center"}}>
        <div style={{fontFamily:"'Space Mono',monospace",fontSize:20,fontWeight:700,color:C.amber,marginBottom:8}}>⬡ STOCKBASE</div>
        <div style={{fontSize:13,color:C.dim,marginBottom:32}}>Enter your team password to continue</div>
        <input
          type="password"
          ...
          style={{width:"100%",boxSizing:"border-box",background:C.bg,border:`1px solid ${passwordError?C.red:C.border}`,color:C.text,borderRadius:8,padding:"11px 14px",fontSize:14,fontFamily:"inherit",marginBottom:12,outline:"none"}}
          autoFocus
        />
        {passwordError && <div style={{color:C.red,fontSize:12,marginBottom:12}}>Incorrect password — try again</div>}
        <button
          style={{width:"100%",padding:"11px",background:C.amber,color:C.bg,border:"none",borderRadius:8,fontSize:14,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}
    ...
    <div style={s.app}>
      ...
      <header style={s.header}>
        ...
        <div style={{ marginLeft:"auto", display:"flex", gap:8 }}>
          {alerts.length > 0 && <div style={{ background:C.dangerBg, border:`1px solid ${C.dangerBorder}`, borderRadius:6, padding:"4px 12px", fontSize:11, color:C.red, fontWeight:700, cursor:"pointer" }} onClick={() => setTab("replenishment")}>⚠ {alerts.length} ALERT{alerts.length!==1?"S":""}</div>}
          <button style={s.btn("secondary")} onClick={exportCSV}>↓ Export</button>
          <button style={{...s.btn("secondary"), fontSize:14}} onClick={() => setTheme(t => t === "dark" ? "light" : "dark")} title="Toggle light/dark mode">{theme === "dark" ? "☀" : "☾"}</button>
        </div>
      </header>
      ...
      <main style={s.main}>
        ...
        {tab==="inventory" && <div style={s.card}>
          ...
          {addForm && (
            <div style={{background:C.surfaceDeep,border:`1px solid ${C.amber}40`,borderRadius:10,padding:18,marginBottom:14,display:"grid",gridTemplateColumns:"repeat(3,1fr) repeat(3,100px) auto",gap:8,alignItems:"end"}}>
              ...
              <tbody>
                {filteredInventory.map(item => {
                  ...
                  return <tr key={item.id}>
                    <td style={{...s.td,fontFamily:"monospace",fontSize:12,color:C.btnText}}>{item.sku}</td>
                    ...
                    <td style={{...s.td,fontFamily:"monospace",color:avgCost>0?C.green:C.muted}}>{avgCost>0?`$${avgCost.toFixed(2)}`:"—"}</td>
        ...
        {tab==="replenishment" && (()=>{
          ...
          const URG = theme === 'dark' ? {
            critical: { label:"CRITICAL",  color:"#f87171", bg:"#450a0a", border:"#7f1d1d", dot:"#ef4444", desc:"Out of stock — no units available" },
            low:      { label:"LOW STOCK", color:"#fb923c", bg:"#451a03", border:"#92400e", dot:"#f97316", desc:"Below reorder point" },
            watch:    { label:"WATCH",     color:"#facc15", bg:"#422006", border:"#854d0e", dot:"#eab308", desc:"Fewer than 4 weeks of runway at current velocity" },
          } : {
            critical: { label:"CRITICAL",  color:"#dc2626", bg:"#fef2f2", border:"#fca5a5", dot:"#dc2626", desc:"Out of stock — no units available" },
            low:      { label:"LOW STOCK", color:"#ea580c", bg:"#fff7ed", border:"#fdba74", dot:"#ea580c", desc:"Below reorder point" },
            watch:    { label:"WATCH",     color:"#ca8a04", bg:"#fefce8", border:"#fde047", dot:"#ca8a04", desc:"Fewer than 4 weeks of runway at current velocity" },
          };
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 320px",gap:18,alignItems:"start"}}>
                      ...
                      <table style={s.table}>
                        ...
                        <tbody>
                          {replRows.map(item=>{
                            ...
                            const wksColor = item.wks < 1 ? C.red : item.wks < 2 ? C.orange : item.wks < 4 ? "#eab308" : C.green;

                            return <tr key={item.id} style={{background:checked?"#a78bfa08":"transparent",transition:"background 0.15s"}}>
                              {/* Checkbox */}
                              ...
                              <td style={{...s.td,fontFamily:"monospace",color:item.avgCost>0?C.green:C.muted,fontSize:12}}>{item.avgCost>0?`$${item.avgCost.toFixed(2)}`:"—"}</td>
                              {/* Open POs column */}
                              <td style={s.td}>
              ...
              <div style={{display:"flex",flexDirection:"column",gap:14}}>
                ...
                <div style={{...s.card,border:selectedCount>0?`1px solid ${C.purple}40`:undefined}}>
                  ...
                  {selectedCount === 0
                    ...
                    : <>
                        ...
                        {(()=>{
                          ...
                          return <div style={{background:C.surfaceDeep,borderRadius:8,padding:"12px 14px",marginBottom:14}}>
                            ...
                            {totalCost>0&&<div style={{display:"flex",justifyContent:"space-between",borderTop:`1px solid ${C.border}`,paddingTop:6,marginTop:2}}>
                              ...
                              <span style={{fontFamily:"monospace",fontWeight:800,color:C.green}}>${totalCost.toLocaleString("en-US",{minimumFractionDigits:2,maximumFractionDigits:2})}</span>
                            </div>}
                          </div>;
                        })()}
                  ...
                  {replCreatedMsg && (
                    <div style={{marginTop:12,padding:"10px 12px",background:STATUS_STYLES.ok.bg,border:`1px solid ${STATUS_STYLES.ok.dot}`,borderRadius:8,fontSize:12,color:C.green,fontWeight:600}}>
            ...
            return <>
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:18,marginBottom:18}}>
                <div style={s.card}>
                  ...
                  {po.notes&&<div style={{marginTop:14,padding:"10px 14px",background:C.surfaceDeep,borderRadius:8,fontSize:13,color:C.btnText,borderLeft:`2px solid ${C.border}`}}>{po.notes}</div>}
                </div>
                <div style={s.card}>
                  ...
                  {po.status==="received"
                    ...
                    :<div style={{color:C.btnText,fontSize:13}}>Use "Receive" on line items below to log incoming stock. Inventory updates automatically.</div>}
                </div>
              </div>
              <div style={s.card}>
                <div style={s.secTitle}>Line Items</div>
                <table style={s.table}>
                  ...
                  <tbody>
                    {po.lines.map((line,idx)=>{
                      ...
                      return <tr key={idx}>
                        <td style={{...s.td,fontFamily:"monospace",fontSize:12,color:C.btnText}}>{line.sku}</td>
        ...
        {tab==="forecast" && (()=>{
          ...
          return <>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr 1fr",gap:14,marginBottom:18}}>
              ...
              <div style={s.card}>
                ...
                {[["all","All Available History","Uses every week of sales data uploaded so far"],["yoy","Year over Year","Matches same calendar weeks from the prior year — great for seasonal businesses"]].map(([id,label,desc])=>(
                  <div key={id} onClick={()=>setFcWindow(id)} style={{padding:"10px 12px",borderRadius:8,cursor:"pointer",marginBottom:8,background:fcWindow===id?"#60a5fa18":C.bg,border:`1px solid ${fcWindow===id?C.blue:C.border}`,transition:"all 0.15s"}}>
                  ...
                  {monthNames.map(m=>(
                    <div key={m}>
                      ...
                      <input
                        ...
                        style={{...s.inp,width:"100%",boxSizing:"border-box",fontSize:12,padding:"5px 6px",color:fcSeasonality[m]>1?C.orange:fcSeasonality[m]<1?"#60a5fa":C.text}}
                      />
                    </div>
                  ))}
                </div>
              ...
              <div style={s.card}>
                ...
                {fcCreatedMsg&&<div style={{marginTop:10,padding:"10px 12px",background:STATUS_STYLES.ok.bg,border:`1px solid ${STATUS_STYLES.ok.dot}`,borderRadius:8,fontSize:12,color:C.green,fontWeight:600}}>
            ...
            <div style={s.card}>
                ...
                <table style={s.table}>
                  ...
                  <tbody>
                    {fcRows.map(item=>{
                      ...
                      const confStyle = theme==='dark'
                        ? { high:{color:"#4ade80",bg:"#052e16"}, medium:{color:C.amber,bg:"#451a0320"}, low:{color:C.red,bg:C.dangerBg} }[confidence]
                        : { high:{color:C.green,bg:"#f0fdf4"}, medium:{color:C.amber,bg:"#fffbeb"}, low:{color:C.red,bg:C.dangerBg} }[confidence];
                      return <tr key={item.id}>
                        <td style={{...s.td,fontFamily:"monospace",fontSize:12,color:C.btnText}}>{item.sku}</td>
                        ...
                        <td style={s.td}>
                          {covered
                            ...
                            :<span style={{fontSize:11,fontWeight:700,color:C.red,background:C.dangerBg,padding:"3px 8px",borderRadius:4}}>⚠ Short {item.forecastUnits-item.currentStock} units</span>}
                        </td>
              ...
              <div style={{marginTop:12,padding:"10px 14px",background:C.surfaceDeep,borderRadius:8,fontSize:11,color:C.dim,lineHeight:1.7,borderLeft:`2px solid ${C.border}`}}>
                <strong style={{color:C.btnText}}>Confidence</strong> is based on data volume: <strong style={{color:C.green}}>High</strong> = 8+ weeks · <strong style={{color:C.amber}}>Medium</strong> = 4–7 weeks · <strong style={{color:C.red}}>Low</strong> = fewer than 4 weeks. Upload more weekly sales CSVs to improve accuracy.
                {fcMethod==="trend"&&<span> · <strong style={{color:C.btnText}}>Trend-Adjusted</strong> uses linear regression — the slope (↑/↓ per week) is shown under each SKU's rate.</span>}
                {fcMethod==="seasonality"&&<span> · <strong style={{color:C.btnText}}>Seasonality</strong> multipliers apply to the months covered by your forecast window. Adjust multipliers in the panel to the left.</span>}
              </div>
            </div>
          </>;
        })()}
            ...
            <div style={{display:"flex",gap:2,marginBottom:22,background:C.surface,borderRadius:10,padding:4,border:`1px solid ${C.border}`,alignSelf:"flex-start",width:"fit-content"}}>
              {[["upload","📤 Upload"],["log","📋 Order Log"],["analytics","📊 Analytics"]].map(([id,label]) => (
                <button key={id} onClick={() => setUploadSubTab(id)} style={{
                  ...
                  color: uploadSubTab===id ? C.bg : C.btnText,
                  transition:"all 0.15s",
                }}>{label}{id==="log"&&allOrderIds.length>0&&<span style={{marginLeft:6,fontSize:10,background:uploadSubTab==="log"?C.bg+"40":C.border,color:uploadSubTab==="log"?C.bg:C.dim,padding:"1px 6px",borderRadius:99}}>{allOrderIds.length}</span>}</button>
              ))}
            </div>
              ...
              {uploadStep==="idle" && <div style={{display:"grid",gridTemplateColumns:"3fr 2fr",gap:18}}>
                <div style={s.card}>
                  ...
                  >
                    ...
                    <div style={{fontSize:16,fontWeight:700,color:C.btnText,marginBottom:8}}>Drop CSV here or click to browse</div>
                  ...
                  {uploadFeedback && (
                    <div style={{marginTop:14,padding:"13px 16px",borderRadius:8,background:C.dangerBg,border:`1px solid ${C.dangerBorder}`,color:C.red,fontSize:13,fontWeight:600,display:"flex",alignItems:"flex-start",gap:10}}>
                ...
                <div style={{display:"flex",flexDirection:"column",gap:14}}>
                  <div style={s.card}>
                    ...
                    <div style={{background:C.surfaceDeep,borderRadius:8,padding:16,fontFamily:"monospace",fontSize:12,color:C.btnText,lineHeight:2,marginBottom:14}}>
                ...
                return <div style={{display:"flex",flexDirection:"column",gap:16}}>
                  ...
                  <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:14}}>
                    <div style={s.statCard("#60a5fa")}>
                      ...
                      <div style={{fontSize:13,fontWeight:700,color:"#60a5fa",fontFamily:"monospace",wordBreak:"break-all",lineHeight:1.3,marginBottom:4}}>{fileName.length > 22 ? fileName.slice(0,22)+"…" : fileName}</div>
                    ...
                    <div style={s.statCard(C.green)}>
                      ...
                      <div style={s.statV(C.green)}>{orderIds.length}</div>
                      <div style={s.statS}>{validLines.length} line items · {previewUnits} units</div>
                    </div>
                    <div style={s.statCard(errorLines.length > 0 ? C.red : C.green)}>
                      <div style={s.statL}>Errors</div>
                      <div style={s.statV(errorLines.length > 0 ? C.red : C.green)}>{errorLines.length}</div>
                      <div style={s.statS}>{errorLines.length > 0 ? "Will be skipped" : "No issues"}</div>
                    </div>
                    <div style={s.statCard(dupLines.length > 0 ? C.orange : C.green)}>
                      <div style={s.statL}>Duplicates</div>
                      <div style={s.statV(dupLines.length > 0 ? C.orange : C.green)}>{dupLines.length}</div>
                      ...
                      <div style={{maxHeight:360,overflowY:"auto",display:"flex",flexDirection:"column",gap:6}}>
                        {orderIds.map(ordId => {
                          ...
                          return <div key={ordId} style={{borderRadius:8,border:`1px solid ${C.border}`,overflow:"hidden"}}>
                            <div onClick={() => setOrderLogExpanded(exp ? null : ordId)} style={{display:"flex",alignItems:"center",gap:12,padding:"10px 14px",cursor:"pointer",background:exp?C.border:C.bg,transition:"background 0.15s"}}>
                            ...
                            {exp && <div style={{background:C.surfaceDeep}}>
                              <table style={s.table}>
                                <thead><tr>
                                ...
                                <tbody>{oLines.map((l,i) => (
                                  <tr key={i}>
                                    <td style={{...s.td,fontFamily:"monospace",fontSize:12,color:C.btnText,padding:"7px 14px"}}>{l.sku}</td>
                    ...
                    <div style={{display:"flex",flexDirection:"column",gap:14}}>
                      {errorLines.length > 0 && <div style={s.card}>
                        ...
                        <div style={{maxHeight:200,overflowY:"auto",display:"flex",flexDirection:"column",gap:6}}>
                          {errorLines.map((e,i) => (
                            <div key={i} style={{padding:"8px 12px",borderRadius:6,background:C.dangerBgSubtle,border:`1px solid ${C.dangerBorderSubtle}`,fontSize:12}}>
                              <div style={{display:"flex",justifyContent:"space-between",marginBottom:3}}>
                                <span style={{fontFamily:"monospace",color:C.muted,fontSize:10}}>Row {e.row}</span>
                                {e.skuRaw && <span style={{fontFamily:"monospace",color:C.btnText,fontSize:10}}>{e.skuRaw}</span>}
                              </div>
                      ...
                      {dupLines.length > 0 && <div style={s.card}>
                        ...
                        <div style={{maxHeight:200,overflowY:"auto",display:"flex",flexDirection:"column",gap:4}}>
                          {[...new Set(dupLines.map(d => d.orderId))].map(ordId => (
                            <div key={ordId} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"7px 10px",borderRadius:6,background:C.warnBgSubtle,border:`1px solid ${STATUS_STYLES.low.dot}40`,fontSize:12}}>
                      ...
                      {errorLines.length === 0 && dupLines.length === 0 && (
                        <div style={{...s.card,textAlign:"center",padding:"32px 16px",border:"1px solid #166534"}}>
                          <div style={{fontSize:28,marginBottom:8}}>✅</div>
                          <div style={{fontSize:13,fontWeight:700,color:C.green,marginBottom:4}}>Clean Import</div>
                        ...
                        <div style={{maxHeight:180,overflowY:"auto",display:"flex",flexDirection:"column",gap:4}}>
                          {(()=>{
                            ...
                            return Object.values(impact).map(({sku,name,deduct}) => {
                              ...
                              return <div key={sku} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"6px 8px",borderRadius:6,background:willOut?C.dangerBgSubtle:willLow?C.warnBgSubtle:"transparent",fontSize:12}}>
                                <div>
                                  <div style={{fontFamily:"monospace",fontSize:10,color:C.btnText}}>{sku}</div>
                                ...
                                <div style={{display:"flex",alignItems:"center",gap:8,textAlign:"right"}}>
                                  ...
                                  {(willOut||willLow) && <span style={{fontSize:9,fontWeight:700,color:willOut?C.red:C.orange,background:willOut?C.dangerBg:STATUS_STYLES.low.bg,padding:"1px 5px",borderRadius:3}}>{willOut?"OUT":"LOW"}</span>}
                                </div>
                              </div>;
                            });
                          })()}
                  ...
                  {/* Commit bar */}
                  <div style={{...s.card,display:"flex",justifyContent:"space-between",alignItems:"center",borderColor:validLines.length>0?STATUS_STYLES.ok.dot:C.border}}>
                    <div>
              ...
              {uploadStep==="committed" && uploadPreview && (()=>{
                ...
                return <div style={{display:"grid",gridTemplateColumns:"2fr 1fr",gap:18}}>
                  <div style={{...s.card,textAlign:"center",padding:"52px 32px",border:"1px solid #166534"}}>
                    ...
                    <div style={{fontSize:22,fontWeight:800,color:C.green,marginBottom:8}}>Upload Complete</div>
                    ...
                    {errorLines.length > 0 && <div style={{marginBottom:20,padding:"10px 14px",background:C.dangerBgSubtle,border:`1px solid ${C.dangerBorder}`,borderRadius:8,fontSize:12,color:C.orange}}>{errorLines.length} row{errorLines.length!==1?"s":""} had errors and were skipped</div>}
                  ...
                  <div style={{display:"flex",flexDirection:"column",gap:14}}>
                    <div style={s.card}>
                      ...
                      {[
                        ["Orders imported", orderIds.length, C.green],
            ...
            {uploadSubTab==="log" && <>
              <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:14,marginBottom:18}}>
                {[
                  ...
                  ["Total Line Items", orders.length, C.green],
                  ...
                  : <div style={{maxHeight:600,overflowY:"auto",display:"flex",flexDirection:"column",gap:5}}>
                      {filteredOrderIds.map(ordId => {
                        ...
                        return <div key={ordId} style={{borderRadius:8,border:`1px solid ${exp?C.border+"80":C.border}`,overflow:"hidden",transition:"all 0.15s"}}>
                          <div onClick={() => setOrderLogExpanded(exp ? null : ordId)} style={{display:"grid",gridTemplateColumns:"1fr 80px 80px 110px 30px",gap:12,alignItems:"center",padding:"11px 16px",cursor:"pointer",background:exp?C.border:C.bg}}>
                          ...
                          {exp && <div style={{background:C.surfaceDeep}}>
                            <table style={s.table}>
                              ...
                              <tbody>
                                {oLines.map((l,i) => (
                                  <tr key={i}>
                                    <td style={{...s.td,fontFamily:"monospace",fontSize:12,color:C.btnText,padding:"8px 16px"}}>{l.sku}</td>
                ...
                : <>
                    <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:14,marginBottom:18}}>
                      {[
                        ...
                        ["Total Line Items",orders.length,      C.green,"individual SKU rows"],
                          ...
                          {topSkusByFreq.map(({sku, orderCount, units, lineItems}, i) => {
                            ...
                            return <div key={sku} style={{display:"flex",alignItems:"center",gap:10,padding:"9px 0",borderBottom:`1px solid ${C.border}`}}>
                              ...
                              <div style={{flex:1,minWidth:0}}>
                                ...
                                <div style={{fontFamily:"monospace",fontSize:10,color:C.btnText,marginBottom:4}}>{sku}</div>
                        ...
                        <tbody>
                          {weeklyOrderCounts.map(({week,orders:cnt,units,lineItems}) => (
                            <tr key={week}>
                              ...
                              <td style={{...s.td,fontFamily:"monospace",color:C.btnText}}>{lineItems}</td>
            ...
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
              ...
              {(()=>{
                ...
                return costed.length>0&&<div style={{textAlign:"right"}}>
                  ...
                  <div style={{fontSize:22,fontWeight:800,fontFamily:"monospace",color:C.green}}>${total.toLocaleString("en-US",{minimumFractionDigits:2,maximumFractionDigits:2})}</div>
                </div>;
              })()}
            </div>
            <table style={s.table}>
              ...
              <tbody>
                {[...inventory].sort((a,b)=>(b.avgCost||0)*b.currentStock-(a.avgCost||0)*a.currentStock).map(item=>{
                  ...
                  return <tr key={item.id}>
                    ...
                    <td style={{...s.td,fontFamily:"monospace",color:item.avgCost>0?C.green:C.muted}}>{item.avgCost>0?`$${item.avgCost.toFixed(4)}`:"—"}</td>
                    ...
                    <td style={s.td}>{item.avgCost>0?<span style={{fontSize:10,fontWeight:700,color:C.green,background:STATUS_STYLES.ok.bg,padding:"2px 7px",borderRadius:4,letterSpacing:1}}>WAC</span>:<span style={{fontSize:11,color:C.muted}}>—</span>}</td>
                  </tr>;
                })}
              </tbody>
            </table>
            <div style={{marginTop:14,padding:"10px 14px",background:C.surfaceDeep,borderRadius:8,fontSize:11,color:C.dim,borderLeft:`2px solid ${C.border}`,lineHeight:1.7}}>
              <strong style={{color:C.btnText}}>WAC (Weighted Average Cost)</strong> — recalculated on every PO receipt: <span style={{fontFamily:"monospace",color:C.amber}}>new avg = (units on hand × old avg cost + received qty × PO unit cost) ÷ new total units</span>. Avg cost resets to the PO price if stock was at zero.
            </div>
          </div>
        </>}
        ...
        {tab==="admin" && <div style={{display:"flex",flexDirection:"column",gap:18}}>
          <div style={s.card}>
            ...
            <div style={{display:"flex",flexWrap:"wrap",gap:8}}>
              {categories.map(cat => (
                <div key={cat} style={{display:"flex",alignItems:"center",gap:6,background:C.surfaceDeep,border:`1px solid ${C.border}`,borderRadius:8,padding:"6px 12px"}}>
            ...
            {importModal.error ? (
              ...
              return <>
                ...
                <div style={{display:"flex",gap:10,marginBottom:14,flexWrap:"wrap"}}>
                  {newCount>0 && <span style={{background:C.successBg,color:C.green,borderRadius:6,padding:"3px 10px",fontSize:11,fontWeight:700}}>{newCount} New</span>}
                  {updateCount>0 && <span style={{background:C.blueBg,color:C.blue,borderRadius:6,padding:"3px 10px",fontSize:11,fontWeight:700}}>{updateCount} Update</span>}
                  {errors.length>0 && <span style={{background:C.dangerBg,color:C.red,borderRadius:6,padding:"3px 10px",fontSize:11,fontWeight:700}}>{errors.length} Error</span>}
                </div>
                  ...
                  <table style={{...s.table,margin:0}}>
                    ...
                    <tbody>
                      {importModal.rows.map((r,i)=>(
                        <tr key={i} style={r.error?{background:C.errorRowBg}:{}}>
                          <td style={{...s.td,fontFamily:"monospace",fontSize:12,color:C.btnText}}>{r.sku||"—"}</td>
                          ...
                          <td style={s.td}>
                            {r.error
                              ...
                              : r.action==="new"
                                ? <span style={{color:C.green,fontSize:11,fontWeight:700}}>New</span>
          ...
          <div style={s.modal} onClick={e=>e.stopPropagation()}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:10,marginBottom:20}}>
              {[["Ordered",receiveModal.po.lines[receiveModal.lineIdx].qty],["Received",receiveModal.po.lines[receiveModal.lineIdx].received],["Remaining",receiveModal.po.lines[receiveModal.lineIdx].qty-receiveModal.po.lines[receiveModal.lineIdx].received]].map(([l,v])=>(
                <div key={l} style={{background:C.surfaceDeep,borderRadius:8,padding:12,textAlign:"center"}}>
